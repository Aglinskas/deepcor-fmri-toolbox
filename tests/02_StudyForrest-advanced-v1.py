...
def _(deepcor, os, output_dir, plt, r, s, track_files):
    ...
    for this_track in tracks:
        try:
            # V1 dashboard takes only `track` and returns a fresh fig each call;
            # close the previous one so we don't accumulate figures in the loop.
            if this_fig is not None:
                plt.close(this_fig)
            this_fig = deepcor.visualization.show_dahsboard_v1_marimo(this_track)
        except Exception as e:
            print(f'bad track: {e}')
    if this_fig is not None:
