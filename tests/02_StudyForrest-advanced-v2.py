...
training_config = deepcor.TrainingConfig(
    ...
    eps=1e-08,
    max_grad_norm=5.0,
    n_repetitions=param_repetitions  # Number of ensemble repetitions
)
...
for ensemble in range(config.training.n_repetitions):
    try:
        ...
        for epoch in range(config.training.n_epochs):
            ...
            fig = deepcor.visualization.show_dahsboard_v2_jupyter(track)
            plt.close(fig)
...
for this_track in tracks:
    try:
        this_fig = deepcor.visualization.show_dahsboard_v2_jupyter(this_track,fig=this_fig,save_fig=False)
    except Exception as e:
        print(f'bad track: {e}')
plt.close(this_fig)
